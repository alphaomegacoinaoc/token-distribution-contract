    ...
    UUPSUpgradeable
{
    ...
    function _transfer(
        ...
        require(
            ...
            "AOC: Cant't transfer, User is blacklisted"
        );
        ...
        if (includedInLTAF[sender] || !excludedFromRAMS[sender]) {
            ...
            if (includedInLTAF[sender]) {
                ...
                require(
                    amount <=
                        ((userInfo[sender].balance * ltafPercentage) / 10 ** 2),
                    "BEP20: Amount is higher than LTAF percentage"
                );
                    ...
                    require(
                        amount <=
                            ((userInfo[sender].balance *
                                levels[userInfo[sender].level].percentage) /
                                10 ** 2),
                ...
                require(
                    txPerMonth[sender][year][month] + amount <=
                        ((userInfo[sender].balance *
                            levels[userInfo[sender].level].percentage) /
                            10 ** 2),
                    "BEP20: Amount exceeds the percentage of this month"
                );
                txPerMonth[sender][year][month] += amount;
            }
        }
    ...
    function bulkTransfer(
        ...
        uint256 senderBalanceAmount = _balances[msg.sender];
        ...
        _balances[msg.sender] = senderBalanceAmount;
        emit BulkTransfer(msg.sender, recipients, amounts);
    }

    function bulkDistribution(
